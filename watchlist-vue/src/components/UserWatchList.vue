    ...
    <div class="wathhlist-top-bar bg-slate-100 overflow-hidden">
      <Transition mode="out-in" enter-active-class="transform transition duration-150 ease-out"
        ...
        <div v-else-if="isEditing" class="flex gap-2 justify-between">
          <button @click="cancelEditing" class="p-2 hover:bg-slate-300 text-4xl aspect-square">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 11H7.83L13.42 5.41L12 4L4 12L12 20L13.41 18.59L7.83 13H20V11Z" fill="#42474E" />
            </svg>
          </button>
          <button @click="applyEditingChanges" class="p-2 hover:bg-slate-300 text-4xl aspect-square">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-else class="flex gap-2">
          <button @click="toggleAddMenu" class="p-2 hover:bg-slate-300 text-4xl aspect-square">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 13H13V20H11V13H4V11H11V4H13V11H20V13Z" fill="#3F3F3F" />
            </svg>
          </button>
          <button @click="openEditing" class="p-2 hover:bg-slate-300 text-4xl aspect-square">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <Transition mode="out-in" enter-active-class="transform transition duration-300 ease-out"
          ...
          <draggable v-else v-model="userStocksToShow" group="stocks" item-key="symbol" handle=".handle">
            <template #header>
              ...
              <div
                class="px-4 py-2 border-y md:flex hidden gap-2 md:gap-20 md:items-center font-sans text-xs font-normal leading-none">
              ...
              <WatchListStock :stock="stock">
                <template #left>
                  <Transition mode="out-in" enter-active-class="transform transition duration-300 ease-out"
                    ...
                    <button class="hover:bg-slate-100" v-if="isEditing" @click="removeUserStock(stock)">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <Transition mode="out-in" enter-active-class="transform transition duration-300 ease-out"
                    ...
                    <div v-else class="watchlist-item-right md:w-full">
                      <div class="flex md:flex-nowrap flex-wrap md:items-center justify-end md:justify-between md:gap-10">
                        ...
                        <div class="md:flex hidden md:w-24">{{ formatter.format(stock.market_cap) }}</div>
                        <div><a
                            class="px-4 py-1 md:flex hidden text-white bg-[#12A600] font-sans text-xs font-bold rounded">
                            TRADE</a></div>
                      </div>
                    </div>
                  </Transition>
...
const userStocksToShow = computed({
  get() {
    if (selectedStocksType.value === 'All') return userStocks.value
    return userStocks.value.filter((s) => s.type === selectedStocksType.value)
  },
  set(val) {
